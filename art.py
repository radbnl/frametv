    ...
    if art_mode == True:
        ...
        if len(files) == 0:
            ...
            for file in files:
                ...
                last_uploaded = remote_filename

                logging.info(f'Uploaded: {file} -> {remote_filename}')

            # Select the last uploaded image
            if last_uploaded:
                tv.art().select_image(last_uploaded, show=True)
                logging.info(f'Selected last uploaded image.')
